...
\begin{document}
\begin{abstract}
We give an explicit decomposition of $\Ind(1)_{B_n}^{S_{2n}}$, following Barbasch and Vogan \cite{bv}.
We define two natural generalizations of $B_n$, and extend the proof in \cite{bv} to recursively compute these decompositions.
...
The algebraic results are a first step in deciding whether the matching polytope has a small equivariant semidefinite lift.
\end{abstract}
\maketitle

\section{Introduction} \label{BV:Introduction}
\subsection{Lifts of polytopes}
...
\subsection{Decomposing $\MATCH_n$ into irreducibles}
